...
MORNING_ACTIONS = [
    "先喝水、吃點東西、拉開窗簾；今天只定一條主線。",
    "先顧身體再開工：喝水、早餐，確認第一個最小成果。",
]
WORK_FOCUS_ACTIONS = [
    "只推一個 owner 明確、可驗證的交付；先寫完成標準。",
    "主線只留一個 DRC/LVS/版圖 closure；過程留 evidence。",
]
WORK_CLOSE_ACTIONS = [
    "停止擴張；保存成果、記卡點與下一步，不替別人接 ownership。",
    "先做 closure；把 owner、scope、證據寫清楚，不默認加班。",
]
STUDY_FOCUS_ACTIONS = [
    "交大先修只推一題或一小節；先寫已知、未知與完成標準。",
    "只讀一頁、算一題或整理一張卡；先求能回想，不求看完。",
]
STUDY_CLOSE_ACTIONS = [
    "停止開新章；標記卡點、寫下一題入口，讓下輪不用重開機。",
    "把筆記收成一個可複習單位；Notion/OCR 不搶走課業主線。",
]
LUNCH_ACTIONS = [
    "先吃飯、喝水、離屏走 5 分鐘；休息不是落後。",
    "午休先補能量與放鬆肩頸；下午只重啟一條主線。",
]
DAY_END_ACTIONS = [
    "記下完成、卡點、明天第一步後停工；晚上留給吃飯、清潔、陪貓與休息。",
    "關掉工作線：保存、留一句下一步，不把白天的壓力拖進晚上。",
]
